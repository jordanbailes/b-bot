...
pub fn parse_chat_response(backend: &'static str, body: &str) -> Result<ChatResponse> {
    let parsed =
        serde_json::from_str::<ChatResponse>(body).map_err(|source| AppError::PlannerParse {
            backend,
            source,
            body: body.to_owned(),
        })?;
    ...
    if parsed.message.content.is_none() && parsed.message.tool_calls.is_none() {
        return Err(AppError::InvalidPlannerResponse { backend });
    }
